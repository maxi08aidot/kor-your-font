...
const { requiredComponents, placeComponent, composeSyllable, decomposeSyllable } = require('./hangul');
...
async function loadComponents(dir, { smooth = 1, weight = 0 } = {}) {
  ...
  for (const blob of blobs.blobs) {
    ...
    if (!label || !required.has(label) || parts[label]) continue;
    const png = await adjustWeight(fs.readFileSync(path.join(dir, blob.crop)), weight);
    const d = await trace(png, { smooth });
    if (!d) continue;
    parts[label] = { d: placeComponent(d, blob.cropSize, blobs.pad), blob };
  }
  return parts;
}
...
function manifestFor(name, parts, text) {
  ...
  for (const char of new Set([...text])) {
    ...
    try {
      ...
      continue; // not a modern syllable - spaces, Latin, punctuation
    }
    if (!piece) continue;
    try {
